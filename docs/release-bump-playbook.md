...
Open `release-assets-summary.md` first when you only need the human-readable checksum table; it should also carry the starter bundle, examples/consumer-repo, first-run story, and Show and tell handoff for safe reuse. Run `corepack pnpm release:assets:smoke -- --dir "$assets_dir" --summary-out "$assets_dir/release-assets-smoke-summary.md"` before reusing tarballs. The smoke command verifies the manifest checksums, checks the summary boundary text, unpacks the CLI tarball to check the package README npm boundary, unpacks the demo audit bundle, confirms the report order `share-summary.md`, then `scorecard.md`, then `recommendations.md`, and checks the compiled site release entrypoints. Review `release-assets-smoke-summary.md` as maintainer evidence for the downloaded bundle, not as standalone adoption proof. After the smoke run, confirm its `Release review path` line says to open `release-assets-summary.md`, then the demo audit `share-summary.md`, then `scorecard.md`, then `recommendations.md`. In the Release Distribution workflow summary, review the `Release asset manifest verified` and `Release asset smoke check passed` sections before reusing downloaded release assets.
