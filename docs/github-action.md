...
The checked-in starter workflow pins the current stable Action release, `YSCJRH/ai-visibility-auditor@v0.3.6`; after a newer release, update that pin only after reviewing the release notes.
...
jobs:
  audit:
    ...
    steps:
      ...
      - id: answerlens
        uses: YSCJRH/ai-visibility-auditor@v0.3.6
        with:
          command: audit
...
Keep the workflow pinned to a reviewed stable release tag, currently `v0.3.6`, instead of a branch or floating ref.
...
- The root Action provisions pnpm with Corepack, so external repositories that call a reviewed release tag such as `YSCJRH/ai-visibility-auditor@v0.3.6` do not need to add `pnpm/action-setup` separately.
