...
Keep the public evidence trail in [self-dogfood-log.md](self-dogfood-log.md). The `v0.3.3` activation umbrella [#33](https://github.com/YSCJRH/ai-visibility-auditor/issues/33) records the first hardening pass, `v0.3.4` records the redirect, locale, and CJK follow-up, `v0.3.5` records the release-asset parity pass, and `v0.3.6` records the release-smoke evidence pass; after that, open a focused issue only when a specific repo-editable gap needs follow-up.
