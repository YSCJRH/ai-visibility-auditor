...
AnswerLens has enough product surface to be useful today. The activation and adoption hardening workstream shipped through `v0.3.3`, with `v0.3.4` tightening the self-dogfood crawler, locale, and starter-pin follow-through, `v0.3.5` realigning release assets with the post-deploy Pages polish, and `v0.3.6` making release asset smoke evidence reviewable before reuse; the ongoing work is to keep that path honest, repeatable, and easy to copy without expanding the product boundary.
