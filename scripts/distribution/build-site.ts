      ...
      body: {
        ...
        <section class="section grid">
          ...
          ${renderPanel("Release asset checklist", "What to download", `<p>Use release assets as the second public front door after the demo, fixture, real-site audit, and Action path make sense.</p><ul>${renderList([
            ...
            "<strong>release-assets-manifest.json</strong>: verify downloaded asset sizes and checksums before reusing tarballs.",
            "<strong>release-assets-summary.md</strong>: read the verified asset table, release review path, starter-bundle.md handoff, examples/consumer-repo link, first-run story template, Show and tell Discussion form, and public sharing boundary before forwarding or reusing downloaded assets."
          ])}</ul><p>After downloading, run the release asset smoke check from a local checkout and read <code>release-assets-smoke-summary.md</code>. Its <code>Release review path</code> should point you to <code>release-assets-summary.md</code>, then the demo audit <code>share-summary.md</code>, then <code>scorecard.md</code>, then <code>recommendations.md</code>.</p><p>Use the smoke summary as maintainer review evidence, not as standalone adoption proof.</p><p>Do not include API keys, private analytics, or raw provider payloads in public release reviews, PRs, issues, release notes, or Discussions.</p><p>If <code>npm view @answerlens/cli</code> returns <code>404</code>, keep using release assets or a local checkout; do not present npm as activated.</p>`)}
        ...
        <section class="section grid">
          ...
          ${renderPanel("release assets 检查清单", "下载什么", `<p>把 release assets 当作第二个公开入口：先看演示、跑 fixture、审计真实站点、理解 Action 路径，再下载固定版本资源。</p><ul>${renderList([
            ...
            "<strong>release-assets-manifest.json</strong>：复用 tarball 前，先核对下载文件大小和 checksum。",
            "<strong>release-assets-summary.md</strong>：转发或复用下载资源前，先读取 verified asset table、release review path、starter-bundle.md handoff、examples/consumer-repo 链接、first-run story template、Show and tell Discussion form 和公开分享边界。"
          ])}</ul><p>下载后，在本地 checkout 里运行 release asset smoke check，并读取 <code>release-assets-smoke-summary.md</code>。其中的 <code>Release review path</code> 应该指向 <code>release-assets-summary.md</code>，然后是 demo audit 的 <code>share-summary.md</code>、<code>scorecard.md</code>、<code>recommendations.md</code>。</p><p>smoke summary 只能作为维护者 review 证据，不要把它当作 standalone adoption proof。</p><p>不要在公开 release review、PR、issue、release notes 或 Discussions 里包含 API keys、private analytics 或 raw provider payloads。</p><p>如果 <code>npm view @answerlens/cli</code> 返回 <code>404</code>，继续使用 release assets 或本地 checkout；不要把 npm 描述成已激活。</p>`)}
    ...
    },
    {
      ...
      body: {
        ...
        <section class="section grid">
          ...
          ${renderPanel("PR review packet", "Copy into review", `<img class="starterPreview" src="${escapeHtml(starterPacketPreviewUrl)}" alt="AnswerLens starter packet preview" loading="lazy"><p>Use this after the first CI run so teammates know what to open, what not to publish, and how to share safely if authorized.</p><pre class="markdown">AnswerLens first run:
...
- Boundary: AnswerLens audits public source material. No consumer AI UI scraping. No ranking or answer-placement guarantee.</pre>`)}
        ...
        <section class="section grid">
          ...
          ${renderPanel("PR 审阅包", "复制到审阅里", `<img class="starterPreview" src="${escapeHtml(starterPacketPreviewUrl)}" alt="AnswerLens starter packet preview" loading="lazy"><p>第一次 CI 跑完后，用这一小段告诉团队先看什么、哪些内容不要公开，以及已有授权时如何安全分享。</p><pre class="markdown">AnswerLens first run:
- 先看 share-summary.md，再看 scorecard.md，最后看 recommendations.md。
- 可公开审阅的 artifact：answerlens-report；默认排除 raw/**。
- 安全下一步：如果已有授权，用 first-run story template 和 Show and tell Discussion form 分享。
