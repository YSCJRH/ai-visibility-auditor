...
const messages: Record<Locale, MessageCatalog> = {
  en: {
    ...
    "nav.home": "Home",
    "nav.docs": "Docs",
    "nav.releases": "Download",
    "nav.examples": "Demo",
    "nav.starter": "CI setup",
    "nav.playbooks": "Fixes",
    "nav.github": "GitHub",
    ...
    "footer.distribution": "Built by YSCJRH from repo docs, releases, and reports. No consumer AI UI scraping. No ranking promises.",
    ...
    "brand.siteSummary": "Audit product pages, review the report, and fix what AI assistants cannot read clearly.",
    ...
    "brand.publicShareBoundary": "Keep API keys, private analytics, and raw provider payloads out of public PRs, issues, release notes, and Discussions.",
    "site.demo": "Demo site",
    ...
    "report.next.title": "Next review steps",
    "report.next.scorecard": "Open `scorecard.md` to verify coverage, checks, and score drivers.",
    "report.next.recommendations": "Open `recommendations.md` to turn the top gap into a page-level fix.",
    "report.next.prSnippet": "Use `pr-snippet.md` when you want a copy-ready GitHub block.",
    "report.next.starter": "Use the AnswerLens starter bundle when this workflow is ready to move into another repository: {starterBundleUrl}",
    "report.next.firstRun": "If this first run is safe to discuss, use the first-run story template and Show and tell Discussion form: {firstRunStoryUrl} and {showAndTellUrl}",
  ...
  "zh-CN": {
    ...
    "footer.distribution": "由 YSCJRH 基于仓库文档、发布记录与报告构建。无消费级 AI 界面抓取，不承诺答案面排名。",
    ...
    "brand.siteSummary": "审计产品页面，查看报告，再修正 AI 助手读不清的内容。",
    ...
    "brand.publicShareBoundary": "不要把 API keys、private analytics 或 raw provider payloads 放进公开 PR、issue、release notes 或 Discussions。",
    "site.demo": "演示站点",
    ...
    "report.fixes": "优先修复项",
    "report.next.title": "下一步审阅",
    "report.next.scorecard": "打开 `scorecard.md`，核对覆盖范围、检查项和分数来源。",
    "report.next.recommendations": "打开 `recommendations.md`，把最重要的缺口变成页面级修复。",
    "report.next.prSnippet": "需要可直接复制到 GitHub 的区块时，使用 `pr-snippet.md`。",
    "report.next.starter": "当这套 workflow 准备复制到其他仓库时，使用 AnswerLens starter bundle：{starterBundleUrl}",
    "report.next.firstRun": "如果这次 first run 适合公开讨论，请使用 first-run story template 和 Show and tell Discussion form：{firstRunStoryUrl} 和 {showAndTellUrl}",
